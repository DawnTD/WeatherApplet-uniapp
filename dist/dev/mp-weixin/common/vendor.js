...
const toDisplayString = (val) => {
  return isString$1(val) ? val : val == null ? "" : isArray$4(val) || isObject$1(val) && (val.toString === objectToString$1 || !isFunction(val.toString)) ? JSON.stringify(val, replacer, 2) : String(val);
};
const replacer = (_key, val) => {
  ...
  } else if (isObject$1(val) && !isArray$4(val) && !isPlainObject$1(val)) {
    return String(val);
  }
  return val;
};
...
const hasOwn$3 = (val, key) => hasOwnProperty$1.call(val, key);
...
function removeInterceptorHook(interceptors2, interceptor) {
  ...
  Object.keys(interceptor).forEach((name) => {
    ...
    if (isArray$4(hooks) && isFunction(hook)) {
      remove(hooks, hook);
    }
  });
}
...
function callHook$1(hook, instance, type) {
  callWithAsyncErrorHandling(isArray$4(hook) ? hook.map((h) => h.bind(instance.proxy)) : hook.bind(instance.proxy), instance, type);
}
...
function normalizePropsOptions(comp, appContext, asMixin = false) {
  ...
  if (isArray$4(raw)) {
    ...
    for (const key in raw) {
      ...
      if (validatePropName(normalizedKey)) {
        const opt = raw[key];
        const prop = normalized[normalizedKey] = isArray$4(opt) || isFunction(opt) ? { type: opt } : Object.assign({}, opt);
        if (prop) {
...
function createInvoker(initialValue, instance) {
  const invoker = (e2) => {
    ...
    if (bubbles.includes(e2.type) && !eventSync) {
      ...
      const res = invoke();
      if (e2.type === "input" && (isArray$4(res) || isPromise(res))) {
        return;
      }
      return res;
    }
